...
ENTER - save the curve (now you can start a new one)
DELETE - delete the chosen point
...
class BezierMain:
    def __init__(self, image_path):
        ...
        button_ax = plt.axes([0.68, 0.182, 0.15, 0.04])
        save_button = Button(button_ax, 'Save the curve')
        save_button.on_clicked(self.on_save_button_clicked)

        button_ax3 = plt.axes([0.68, 0.112, 0.15, 0.04])
        change_button = Button(button_ax3, 'Change existing curve')
        ...
        # Adding instruction text to the plot
        self.instruction_text = (
            "Right click - add a new point\n"
            "Left click - select a point, move it\n"
            "Middle click on a point - create new\n curve with this point as start point\n"
            "ENTER - save the curve\n (now you can start a new one)\n"
            "DELETE - delete the chosen point\n"
            "SHIFT - clear everything and start over\n\n"
            "PS: Please remember to press ENTER when\n adding a new curve, or when you want\n to save image."
        )
        
        self.fig.text(0.68, 0.72, self.instruction_text, fontsize=10, verticalalignment='center')
    ...
    def on_save_button_clicked(self, event):
        ...
        text_box_fig.canvas.manager.set_window_title('Save the curve')
        
        text_ax = plt.axes([0.5, 0.5, 0.1, 0.3])
        self.text_box = TextBox(text_ax, 'Enter file name: ')

        def submit_image_name(image_name):
            name = 'curves_png/' + image_name + '.png'
            self.fig.savefig(name)

            path ='curves_txt/' + image_name + '.txt'
            with open(path, 'w') as file:
                for point_group in self.points.values():
                    for point in point_group:
                        file.write(f'{point.x},{point.y}\n')
    ...
    def on_change_button_clicked(self, event):
        ...
        text_box_fig.set_size_inches(6, 1)
        text_box_fig.canvas.manager.set_window_title('Change some')
        
        text_ax = plt.axes([0.45, 0.4, 0.3, 0.3])
        self.text_box = TextBox(text_ax, 'Enter file name:')

        def submit_image_name(letter):
            path_p =  os.path.join('curves_txt/', letter + '.txt')
            if os.path.exists(path_p):
                self.load_points(path_p)
                path_b = 'backgr/empty.png'
    ...
    def add_change_button(self):
        button_ax3 = plt.axes([0.68, 0.112, 0.15, 0.04])
        change_button = Button(button_ax3, 'Change some curve')
        change_button.on_clicked(self.on_change_button_clicked)
